...
<head>
    ...
    <meta name="description" content="{{ $metaDescription ?? 'madPOS authentication' }}">
    <meta name="viewport" content="width=device-width, initial-scale=1">


    <title>{{ $title ?? 'ZerithonLabs' }}</title>
    <link rel="apple-touch-icon" href="{{ asset(config('madpos_ui.favicon')) }}">
    <link rel="icon" href="{{ asset(config('madpos_ui.favicon')) }}">
...
<body>
    <div id="wrapper" class="wrapper-login">
        <div class="login-inner">
            <div class="auth-logo">
                <img src="{{ asset(config('madpos_ui.logo')) }}" alt="madPOS">
            </div>
            <div class="card mb-1">
